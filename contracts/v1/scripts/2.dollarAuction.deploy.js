...
const main = async () => {
  try {
    ...
    const gameContract = await Game.deploy(adminWallet.address, workerWallet.address, tokenAddress);
    await gameContract.waitForDeployment();
    const gameAddress = await gameContract.getAddress();

    await verifyContract({
      address: gameAddress,
      constructorArguments: [adminWallet.address, workerWallet.address, tokenAddress],
    });
    console.log(`========== SUCCESS deployed dollar auction, address: ${gameAddress} ==========`);
    ...
    console.error(err);
    console.log(`========== FAILED deployed dollar auction, error: ${err.message} ==========`);
  }
