...
const useActiveData = () => {
  ...
  const setWinners = useSystemStore((state) => state.setWinners);

  const [activeRoundId, setActiveRoundId] = useState(null);

  useEffect(() => {
    const unsubscribe = onSnapshot(doc(firestore, 'system', 'main'), (snapshot) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(firestore, 'system', 'winners'), (snapshot) => {
  ...
  useEffect(() => {
    let unsubscribe;
    if (activeRoundId) {
      unsubscribe = onSnapshot(doc(firestore, 'rounds', activeRoundId), (snapshot) => {
