...
import { IconETH, IconRUBY, IconGOLD, IconEdit } from './Icons';
import useUser from '../hooks/useUser';
import { formatAmount } from '../utils/strings';

const TabAccount = () => {
  const { user } = useUser();
  const [openModalUpdateUsername, setOpenModalUpdateUsername] = useState(false);
    ...
    <div className="grow overflow-auto p-4 flex flex-col gap-2">
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="transition duration-300 active:scale-90" onClick={() => setOpenModalUpdateUsername(true)}>
            <IconEdit className="w-6 h-6 sm:w-8 sm:h-8" />
          </button>
        </div>
      </div>
      <ConnectWalletButton />
      ...
      <ModalUpdateUsername
        open={openModalUpdateUsername}
        close={() => setOpenModalUpdateUsername(false)}
